...
import { LocalizationProvider } from '@mui/x-date-pickers/LocalizationProvider';
import { AdapterDateFns } from '@mui/x-date-pickers/AdapterDateFns';
import conferenceroom from '../../assets/conferenceroom.png'
...
const BookDesk = () => {
  ...
  const renderEntity = (entity) => {
    ...
    const { type, rotation } = entity;
    const status = entity.seat?.status;
    const xPosition = (parseFloat(entity["x_position"]) || 0) + 250; // Add 100px to x position
    const yPosition = parseFloat(entity["y_position"]) || 0;
  ...
  const renderConferenceRoomImage = () => (
    <img
      src={conferenceroom}
      alt="Conference Room"
      style={{
        ...
        top: '-4850px',
        marginRight: 'px',
        width: '230px', // Adjust the width according to your preference
        height: 'auto',
      }}
    />
  );
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", marginTop: 50, backgroundColor: '#F5F5DC' }}> {/* Beige background */}
      {layout ? (
        layout.original.layout_entities.map((entity) => renderEntity(entity))
      ) : (
        <p></p>
      )}
            {renderConferenceRoomImage()} {/* Render the conference room image */}
